...
'''
  ...
  Examples:
    python update_templates_and_goldens.py
...
def handle_chat_template(template_name, template_src):
    template_file = f'third_party/templates/{template_name}.jinja'
    ...
    context_files = glob.glob('tests/contexts/*.json')
    for context_file in context_files:
        ...
        output_file = f'tests/goldens/{template_name}-{context_name}.txt'
...
def main():
    for dir in ['third_party/templates', 'tests/goldens']:
    ...
    for model_id in model_ids:
        ...
        logging.info(f"# Fetching {model_id}")
        ...
        license = None
        try:
            with open(hf_hub_download(repo_id=model_id, filename="LICENSE")) as f:
                license = f.read()

                license_file = f'third_party/templates/{model_name}.LICENSE'
                logger.info(f'- LICENSE: {license_file}')
        ...
        if isinstance(chat_template, str):
            handle_chat_template(model_name, chat_template)
        else:
            for ct in chat_template:
                variant = ct['name']
                handle_chat_template(f'{model_name}-{variant}', ct['template'])
